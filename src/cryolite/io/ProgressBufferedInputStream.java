...
public class ProgressBufferedInputStream extends FilterInputStream {
   ...
   */
  private static final AtomicReferenceFieldUpdater<ProgressBufferedInputStream, byte[]> bufUpdater = AtomicReferenceFieldUpdater
      .newUpdater(ProgressBufferedInputStream.class, byte[].class, "buf");
  ...
  private int read1(byte[] b, int off, int len) throws IOException {
    ...
    if (avail <= 0) {
      ...
      if (len >= getBufIfOpen().length && markpos < 0) {
        int n = getInIfOpen().read(b, off, len);
        if (n > 0)
          ioProgress.setProgress(n);
        return n;
      }
   ...
   * 
   * @exception IOException
   *              if the stream does not support seek, or if this input stream
   *              has been closed by invoking its {@link #close()} method, or an
   *              I/O error occurs.
   */
